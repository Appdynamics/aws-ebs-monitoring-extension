...
##Use Case
Captures EBS statistics from Amazon CloudWatch and displays them in the AppDynamics Metric Browser.
...
1. Run 'mvn clean install' from aws-ebs-monitoring-extension
2. Copy and unzip AWSEBSMonitor-\<version\>.zip from 'target' directory into \<machine_agent_dir\>/monitors/
3. Edit config.yml file in AWSEBSMonitor/conf and provide the required configuration (see Configuration section)
...
1. If SIM is enabled, then use the following metricPrefix - 
   ...
   `metricPrefix: "Custom Metrics|Amazon EBS|"`
   ...
   `metricPrefix: "Server|Component:<COMPONENT_ID>|Custom Metrics|Amazon EBS|"`.

   For example,
         
    ```
    metricPrefix: "Server|Component:100|Custom Metrics|Amazon EBS|"
    ```
...
3. If you want to encrypt the **awsAccessKey** and **awsSecretKey** then follow the "Credentials Encryption" section and provide the encrypted values in **awsAccessKey** and **awsSecretKey**. Configure `enableDecryption` of `credentialsDecryptionConfig` to `true` and provide the encryption key in `encryptionKey`.
   ...
   ```
4. To report metrics only from specific dimension values, configure the `dimesion` section. Dimensions for AWS EBS are `VolumeId`. 
    ```
    dimensions:
      - name: "VolumeId"
...
## Metrics
Typical metric path: **Application Infrastructure Performance|<Tier>|Custom Metrics|Amazon EBS|<Account Name>|<Region>|Volume Id|<Volume Id>** followed by the metrics defined in the link below:
...
Always feel free to fork and contribute any changes directly via [GitHub](https://github.com/Appdynamics/aws-ebs-monitoring-extension).
...
List of changes to this extension can be found [here](https://github.com/Appdynamics/aws-ebs-monitoring-extension/blob/master/CHANGELOG.md)
